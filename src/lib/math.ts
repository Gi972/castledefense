...
export function generateArcPositions(
  ...
  if (attempts >= maxAttempts) {
    console.warn(`Could only place ${targets.length} targets after ${maxAttempts} attempts.`)
  }
...
export function degToRadStartStop(start: number, end: number): [number, number] {
...
export function generateArcAnnulusPositionsWithDistance(
  ...
  while (positions.length < count && tries < maxTries) {
    ...
    if (tries > maxTries) {
      console.warn('Nombre de tentatives dépassé, résultat partiel:', positions.length)
      break
    }
  }
